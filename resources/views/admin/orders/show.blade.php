<div class="box box-info">
	...
	<div class="box-body">
		<table class="table table-bordered">
			...
			<tr>
				<td>收货地址</td>
				<td colspan="3">{{ implode(' ', $order->address) }}</td>
			</tr>
			<tr>
				<td rowspan="{{ $order->items->count() + 1 }}" style="vertical-align: middle;">商品列表</td>
				<td>商品名称</td>
				<td>单价</td>
				<td>数量</td>
			</tr>
				...
				<tr>
					<td>
						<a href="{{ route('products.show', [$item->product_id]) }}" target="_blank">
							<img src="{{ $item->product->image_url }}" alt="" class="img-responsive thumbnail" width="120px;" >
			...
			@if($order->ship_status === \App\Models\Order::SHIP_STATUS_PENDING)
				<tr>
					<td colspan="4">
						<form action="{{ route('admin.orders.ship', [$order->id]) }}" method="post" class="form form-inline">
							{{ csrf_field() }}
							...
							<div class="form-group {{ $errors->has('express_no') ? 'has-error' : ''}}">
								...
								<input type="text" id="express_no" name="express_no" value="{{ old('express_no') }}" class="form-control" placeholder="输入物流公司">
								@if($errors->get('express_no'))
