		...
		<div class="col-lg-10 col-lg-offset-1">
			<div class="panel panel-default">
				<div class="panel-heading">
					商品评价
					<a href="{{ route('orders.index') }}" class="pull-right">返回商品列表</a>
				</div>
									...
									<td class="product-info">
										<div class="preview">
											<a href="{{ route('products.show', ['product' => $item->product_id]) }}" target="_blank">
												<img src="{{ $item->product->image_url }}" class="img-responsive">
											</a>
										</div>
										<div>
										<span class="product-title">
											<a href="{{ route('products.show', ['product' => $item->product_id]) }}">{{ $item->product->title }}</a>
										</span>
