					...
					<table class="table">
						...
						@foreach($order->items as $index =>  $item)
							<tr>
								<td class="product-info">
									...
									<div>
										<span class="product-title">
											<a href="{{ route('products.show', ['product' => $item->product_id]) }}" target="_blank">{{ $item->product->title }}</a>
										</span>
								...
								<td class="sku-price text-center vertical-middle">{{ number_format($item->price * $item->amount, 2, '.', '') }}</td>
							</tr>
						@endforeach
		...
		$(document).ready(function(){
		    $('#btn-receive').click(function () {
		        swal({
		            ...
		            axios.post('{{ route('orders.receive', ['order' => $order->id]) }}')
			            .then(function () {
			                location.reload();
			            })
		        })
